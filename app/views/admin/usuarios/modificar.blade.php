...
@section('content')
   ...
   {{ Form::open(array('url'=>'admin/usuarios/modificar/'.$usuario->id, 'class'=>'form-signup')) }}
      <h2 class="form-signup-heading">Por Favor Registrate</h2>
    
      
    
      {{ Form::text('username', $usuario->username, array('class'=>'input-block-level', 'placeholder'=>'Nombre Usuario')) }}
      <span class="help-block">{{ $errors->first('username') }}</span>
      {{ Form::text('email', $usuario->email, array('class'=>'input-block-level', 'placeholder'=>'Correo Electronico')) }}
      ...
      {{ Form::select('tipo_user', array(
         'Tipo de Usuario' => array(
            '1' => 'Gestíon', 
            '2' => 'Administración'
         )), array($usuario->tipo_user), array('class' => 'form-control')) }}
